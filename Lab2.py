...
def run_epoch(model: nn.Module, loader: DataLoader, train: bool = True, lr: float = 0.1) -> tuple[torch.tensor, torch.tensor]:
...
def train_model(model: nn.Module, train_loader: DataLoader, test_loader: DataLoader, lr: int = 0.1, n_epochs: int = 1000, print_every: int = 50, early_stopping: bool = True, patience: int = 5000, min_delta: float = 1e-2):
    ...
    for epoch in range(n_epochs):
        # Training over epoch
        train_loss, train_acc = run_epoch(model, train_loader, train=True, lr=lr)

        # Testing over epoch
        test_loss, test_acc = run_epoch(model, test_loader, train=False, lr=lr)
...
# Train the model
model, training_curve = train_model(model, train_loader, test_loader, n_epochs=6, print_every=2)
